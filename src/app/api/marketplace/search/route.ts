...
// GET /api/marketplace/search/developers
export async function GET(request: Request) {
  try {
    ...
    if (searchParams.get('experienceMin')) {
      filters.experienceMin = parseInt(searchParams.get('experienceMin') || '0');
    }
    ...
    const result = await marketplaceService.searchDevelopers(filters);
