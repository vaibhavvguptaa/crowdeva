...
const DevelopersMarketplacePage: React.FC = () => {
  const router = useRouter();
  const [developers, setDevelopers] = useState<DeveloperProfile[]>([]);
  const [loading, setLoading] = useState(true);
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");
  const [sortBy, setSortBy] = useState<
    "relevance" | "rating" | "rate" | "recent" | "experience"
  >("relevance");
  ...
  // Search developers when filters change
  useEffect(() => {
    const searchDevelopers = async () => {
      try {
        ...
        const result = await marketplaceService.searchDevelopers(searchFilters);
        
        // Apply sorting
        let sortedDevelopers = [...result.profiles] as DeveloperProfile[];
        switch (sortBy) {
          case "rating":
            sortedDevelopers.sort((a, b) => b.rating - a.rating);
            break;
          case "rate":
            sortedDevelopers.sort((a, b) => a.hourlyRate - b.hourlyRate);
            break;
          case "experience":
            sortedDevelopers.sort((a, b) => b.experience - a.experience);
            break;
          case "recent":
  ...
  const handleFiltersChange = useCallback(
    (newFilters: Partial<IMarketplaceFilters>) => {
  ...
  const handleViewProfile = (id: string) => {
    router.push(`/marketplace/profile/${id}?type=developer`);
  };
  ...
  const getActiveFiltersCount = () => {
    return (
      ...
      ((filters.hourlyRateMax || 200) < 200 ? 1 : 0) +
        ...
        <div className="mb-6">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search developers by skills, name, or location..."
        ...
        <div className="flex gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              {filterOptions.skills.slice(0, 3).map((skill) => (
                <button
                  key={skill}
                  onClick={() => {
                    const skillFilter = filters.skills?.includes(skill)
                      ? filters.skills.filter((s) => s !== skill)
                      : [...(filters.skills || []), skill];
                    handleFiltersChange({ ...filters, skills: skillFilter });
                  }}
                  className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
...
export default withAuth(DevelopersMarketplacePage, [
  "customers",
  "developers",
  "vendors",
]);